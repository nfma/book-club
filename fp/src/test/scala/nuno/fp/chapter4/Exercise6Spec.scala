...
class Exercise6Spec extends WordSpec with Matchers {
  "Exercise 6" should {
    "implement map" in {
      Left("exception").map(identity) should be(Left("exception"))
      Left("exception").map(_ => "good") should be(Left("exception"))
      Right(1).map(identity) should be(Right(1))
    ...
    "implement map2" in {
      ...
      Right(1).map2(Right(2))((_, _) => "another value") should be(Right("another value"))
