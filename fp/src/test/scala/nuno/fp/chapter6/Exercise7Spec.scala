...
class Exercise7Spec extends WordSpec with Matchers {
  "Exercise 7" should {
    "implement sequence" in {
      sequence(List()) {Simple(42)} shouldBe (Nil, Simple(42))
      sequence(List(double _)) {Simple(42)} shouldBe (List(0.007524831689672932), Simple(1059025964525L))
      sequence(List(nonNegativeInt _, double _, double _)) {Simple(42)} shouldBe (List(16159453, 0.5967354856416283, 0.15846728447753344), Simple(259172689157871L))
    }
